// Leaderboard 配置项
global_switch_time = 120; // 计分板轮换时间 (second)

// leaderboard.online_time : floor

// 初始化
__on_start() -> (
    initialize_logger();
    scoreboard_init('leaderboard.mined_block', 'dummy', '§d§l挖掘榜');
    scoreboard_init('ml.total', 'dummy', '§d§l挖掘榜');
    scoreboard_init('leaderboard.used_block', 'dummy', '§a§l建造榜');
    scoreboard_init('leaderboard.online_time', 'dummy', '§e§l在线时长榜§r§7(小时)');
    // scoreboard_init('leaderboard.elytra', 'dummy', '§e§l鞘翅飞行距离榜');
    scoreboard_init('leaderboard.settings', 'dummy', '§l计分板设置');
    
    //scoreboard('leaderboard.settings', 'switch_time', 0);
    //scoreboard('leaderboard.settings', 'switch', 0);
    //run('scoreboard objectives setdisplay sidebar leaderboard.settings');
); 
...
__config() -> {
   ...
   'commands' -> {
        '' -> 'print_info',
		'sync manual' -> 'sync_manual',
        'sync mltotal' -> 'sync_ml_total_data',
        'sync all' -> 'sync_all'
	},
...
initialize_logger() -> (
    ...
    time = str('%02d:%02d:%02d',
        unix_time:3, unix_time:4, unix_time:5
    );

    print('['+time+'] [INFO] '+'App \''+system_info('app_name')+'.sc\' initialized with '+global_switch_time/20+'s switch time');
    // logger('info', 'App \'' + system_info('app_name') + '.sc\' initialized');
); // [HH:MM:SS] [INFO] App XXXXX.sc initialized with {global_switch_time} switch time

print_about() -> (
    //print(player(), '--+======================================+--');
    //print(player(), '   Leaderboard  |  a simple Scarpet script');
    //print(player(), '       by TomatoOfficial  |  2026');
    //print(player(), '--+======================================+--');
    print(player(), '\n-+================================+-\n   Leaderboard | a simple Scarpet script\n          by TomatoOfficial  |  2026\n-+================================+-');
);

print_info() -> (
    print('当前正在轮播以下计分板: \n'+global_scoreboards);
    print('距离计分板下一次切换: '+(120-(global_time/20))+'s/'+global_switch_time/20+'s');
);
...
// global_scoreboards = ['leaderboard.used_block', 'leaderboard.online_time', 'leaderboard.mined_block'];
// global_scoreboards = ['leaderboard.used_block', 'leaderboard.elytra', 'leaderboard.mined_block'];
global_scoreboards = ['leaderboard.used_block', 'leaderboard.mined_block', 'leaderboard.online_time'];
global_player_id = [];

switch_scoreboards() -> (
    global_time += 1;
    //scoreboard('leaderboard.settings', 'switch_time', global_time);

    if(global_time == global_switch_time,
        //scoreboard('leaderboard.settings', 'switch', global_current);
        global_time = 0;
        global_current += 1;
    );

    if(global_current >= 3, global_current = 0);
    
    run('scoreboard objectives setdisplay sidebar '+global_scoreboards:global_current);

);

sync_manual() -> (
    print('此同步方式已过时，请使用/leaderboard sync all同步数据。');
    // sync_statistic_data(player(), 'manual');
);

sync_statistic_data(player, type) -> (
	if (query(player, 'player_type') != 'fake',

        if(type == 'manual',
            // run('tag ' + player + ' remove leaderboard.synced');
            // print('\n你即将进行手动同步统计数据，这可能导致计分板某些数据减少等情况\n再次输入/leaderboard manual sync以手动同步数据');
            print('此同步方式已过时，请使用/leaderboard sync all同步数据。');
        );

		if(!query(player, 'has_scoreboard_tag', 'leaderboard.synced'),
			mined = 0;
			used = 0;
			...
			scoreboard('leaderboard.mined_block', player, mined);
			scoreboard('leaderboard.used_block', player, used);

			run('tag ' + player + ' add leaderboard.synced');
            // print('synced well');

            if(type == 'manual',
                print('你已手动将统计数据同步至计分板!')
            );

            if(type == 'connect',
                print('你的统计数据已自动同步至计分板!')
            );
...
sync_ml_total_data() -> ( // objective, destination, tag
    print('这个同步方式过时了。')
    // player_id = scoreboard('ml.total'); // player_id = [];
    // for(player_id,
    //     mined = scoreboard('ml.total', _);
    //     scoreboard('leaderboard.mined_block', _, mined);
    //     // run('tag ' + _ + ' add leaderboard.mltotal.synced');
    // );
);

sync_all() -> (
    print('好像这个同步方式也过时了（（（')
    // // We can get all the players' name from ml.total
    // global_player_id = scoreboard('ml.total'); // global_player_id = ['player1', 'player2'];
    // print(global_player_id);

    // // Synchronize ml.total -> leaderboard.mined_block
    // for(global_player_id,
    //     scoreboard('leaderboard.mined_block', _, scoreboard('ml.total', _));
    // );

    // // Synchronize statistic_used -> leaderboard.used_block
    // for(global_player_id,

    //     used_count = 0;
    //     player_id = _; // Assign the GLOBAL_PLAYER_ID of the current loop to PLAYER_ID

    //     for(global_block_id,
    //         used_count += statistic(player_id, 'used', _);
    //     ); // Count the amount of blocks placed in global_block_id
    //     scoreboard('leaderboard.used_block', player_id, used_count);
    // );

    // //Synchronize statistic_play)time -> leaderboard.online_time
    // for(global_player_id,
    //     player_id = player(_);
    //     play_time = round(statistic(_, 'custom', 'play_time') / 20 / 60 / 60);
    //     scoreboard('leaderboard.online_time', _, play_time);
    //     print('we got '+play_time+'('+statistic(player_id, 'custom', 'play_time')+') for '+_);
    // );
...
__on_player_breaks_block(player, block) -> (
    if(query(player, 'player_type') != 'fake',
	    scoreboard_incr('leaderboard.mined_block', player, 1);
    );
);

__on_player_places_block(player, item_tuple, hand, block) -> (
    if(query(player, 'player_type') != 'fake',
	    scoreboard_incr('leaderboard.used_block', player, 1);
    );
);

update_online_time(player) -> (
    if(query(player, 'player_type') == 'fake',
        //logger('info', 'update canceled, because player '+player+' is a bot')
        continue;
    , // else
        (   
            play_time = round(statistic(player, 'custom', 'play_time') / 20 / 60 / 60);
            scoreboard('leaderboard.online_time', player, play_time);
        );
    );

);

update_elytra_distance(player) -> (
    if(query(player, 'player_type') != 'fake',
        distance = 0;
        // distance += statistic(player, 'custom', 'fly_one_cm');
        distance += statistic(player, 'custom', 'aviate_one_cm');
        scoreboard('leaderboard.elytra', player, distance);
    );
);
...
__on_tick() -> (
    switch_scoreboards();
    for(player('all'),
        update_online_time(_);
        // update_elytra_distance(_);
    );
);
